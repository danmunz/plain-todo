...
- reorder is implemented in the inbox via keyboard and menu actions, each task view remembers its own display sort mode in app state without mutating file order, sorted views now render lightweight grouping headers, and Cmd+F presents a transient live-search overlay that composes with sidebar filters
- drag reorder, richer diff tooling, and broader regression coverage for end-to-end keyboard editing remain open
