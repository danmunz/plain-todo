...
- reorder is implemented in the inbox via keyboard, menu, and drag actions, each task view remembers its own display sort mode in app state without mutating file order, sorted views render lightweight grouping headers, and Cmd+F presents a transient live-search overlay that composes with sidebar filters
