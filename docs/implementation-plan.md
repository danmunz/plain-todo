...
This repo is no longer docs-only. It now has a SwiftPM bootstrap, an XcodeGen-backed `Plain.xcodeproj`, `PlainCore`, a SwiftUI `PlainApp` shell, parser and serializer coverage, coordinated read and write support, inline raw-line editing, a scratch-pad raw file editor, a global quick-add panel, an optional menu bar mode with live counts and quick actions (currently disabled due to macOS 15 + macOS 26 SDK incompatibility with MenuBarExtra alongside WindowGroup), a WidgetKit extension with AppIntent-based filter configuration, custom `plain://` deep links into filtered views, keyboard row selection, UndoManager-backed destructive edits, keyboard and drag reorder, coordinated archive-to-`done.txt`, a read-only Done view, archive-behavior, creation-date, show-completed, and menu-bar preferences, persisted per-view sort modes, basic last-file and last-filter session restore with NSWindow frame autosave, grouped sorted task views, a transient Cmd+F search overlay with live filtering, natural-language date parsing and preview in the add bar, explicit external-change conflict handling with reload, keep-mine, and basic view-diff flows, multi-select with bulk complete, delete, and priority set (Cmd+A, Cmd+1/2/3/0, Shift+Arrow extend selection), priority color coding per spec (A=red, B=amber, C=blue), inline syntax highlighting for +project and @context tags with a user-configurable font size slider, contextual empty states per filter, iCloud file-unavailable detection with auto-retry, VoiceOver accessibility labels, a "done this week" ambient stat in the status bar, parser performance regression tests up to 5000 lines, J/K vim-style navigation, hover-revealed row menus with fade-in, humanized due date labels, a transient undo toast for completion and deletion with auto-dismiss, reduced-motion-aware animations (checkbox transition, completion fade), a due-date sort mode with Overdue/Today/Upcoming/No Due Date grouped buckets, Shift+Arrow extend selection for contiguous multi-select, a polished input bar with + icon and ⌘N shortcut hint, a 1-second accent-color highlight pulse for newly added tasks, a three-option onboarding flow (open existing, create new via NSSavePanel, use bundled sample), a tabbed preferences UI (General/Appearance/Shortcuts/Advanced) with theme picker, font size slider, default priority dropdown, .bak backup toggle, and debug logging toggle, and optional .bak backup before write wired through CoordinatedTodoStore. Test coverage stands at 122 tests (84 PlainAppTests + 38 PlainCoreTests). The plan below remains the dependency order for the remaining work.
