...
This repo is no longer docs-only. It now has a SwiftPM bootstrap, an XcodeGen-backed `Plain.xcodeproj`, `PlainCore`, a SwiftUI `PlainApp` shell, parser and serializer coverage, coordinated read and write support, inline raw-line editing, a scratch-pad raw file editor, a global quick-add panel, an optional menu bar mode with live counts and quick actions (currently disabled due to macOS 15 + macOS 26 SDK incompatibility with MenuBarExtra alongside WindowGroup), a WidgetKit extension with AppIntent-based filter configuration, custom `plain://` deep links into filtered views, keyboard row selection, UndoManager-backed destructive edits, keyboard and drag reorder, coordinated archive-to-`done.txt`, a read-only Done view, full preferences system (General/Appearance/Shortcuts/Advanced tabs with theme picker, font size slider, default priority dropdown, .bak backup toggle, debug logging, and file path display with Change button), persisted per-view sort modes, session restore (last file, last filter, window frame, sidebar collapse state), grouped sorted task views, a transient Cmd+F search overlay with live filtering, natural-language date parsing and preview in the add bar, explicit external-change conflict handling with reload, keep-mine, and basic view-diff flows, multi-select with bulk complete, delete, and priority set (Cmd+A, Cmd+1/2/3/0, Shift+Arrow extend selection), priority color coding per spec (A=red, B=amber, C=blue), inline syntax highlighting for +project, @context, and key:value pairs with a user-configurable font size slider, contextual empty states per filter, iCloud file-unavailable detection with auto-retry, VoiceOver accessibility labels, a "done this week" ambient stat in the status bar, parser performance regression tests up to 5000 lines, J/K vim-style navigation, hover-revealed row menus with fade-in, right-click context menus (complete, edit, priority, move, delete), humanized due date labels, a transient undo toast for completion and deletion with auto-dismiss, reduced-motion-aware animations (checkbox transition, completion fade, new-task highlight pulse), a due-date sort mode with Overdue/Today/Upcoming/No Due Date grouped buckets, Shift+Arrow extend selection for contiguous multi-select, a polished input bar with + icon and ⌘N shortcut hint, a three-option onboarding flow (open existing, create new via NSSavePanel, use bundled sample), standard macOS app menus (File > Open, File > New Task, View > Toggle Sidebar, View > Cycle Sort Mode, Help, About), an SF Symbol toolbar (sidebar toggle, sort picker, scratch pad, archive), a window title showing the current filename, subtle row separators, Cmd+\\ sidebar toggle with persisted state, optional .bak backup before write wired through CoordinatedTodoStore, and the Done sidebar item in its own section below a separator per spec. Test coverage stands at 89 tests (88 unit + 1 UI), all passing. The plan below remains the dependency order for the remaining work.
...
- external todo and done file changes are now observed explicitly, with non-modal reload, keep-mine, and basic view-diff conflict handling that avoids clobbering active drafts
- reorder is implemented in the inbox via keyboard, menu, and drag actions, each task view remembers its own display sort mode in app state without mutating file order, normal launches restore the last writable file plus a validated sidebar filter, completed rows stay visible by default until archived with a persisted hide toggle for list cleanup, scratch pad mode now toggles the main detail pane into raw whole-file editing and writes back through the coordinated store, a `Ctrl+Opt+T` quick-add panel can append to the active writable file even when the main window is hidden, the optional menu bar extra reuses the shared shell model for live counts and quick actions, sorted views render lightweight grouping headers, Cmd+F presents a transient live-search overlay that composes with sidebar filters, and the add bar previews and applies supported due-date phrases with automatic creation dates controlled by preferences
