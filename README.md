...
- inline raw-line editing, keyboard row selection, grouped sorted task views, Cmd+F live search, UndoManager-backed destructive edits, reorder, coordinated archive-to-`done.txt`, archive-behavior preferences, and reload-or-keep-mine conflict handling for external file changes
- Xcode-native app, unit-test, and UI-test path
- no database or hidden persistence
- no widget, menu bar, quick-add, diff view, bulk actions, drag-to-reorder, or scratch pad yet